...
Trait UploadTrait
{
    ...
    protected function upload(File $file, SessionInterface $session)
    {
        ...
        if (!$session->has($this->provideKey())) {
            $session->set($this->provideKey(), []);
        }

        $data = $session->get($this->provideKey());

        $data[] = [$newFilename => 0 ];

        $session->set($this->provideKey(), $data);
    }

    protected function getFindPath(SessionInterface $session): string
    {
        return sprintf('%s/%s', $this->getParameter('app.path.image_orphanage'), $this->getPath($session));
    }
