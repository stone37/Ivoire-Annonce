...
class OrphanageManager
{
    ...
    public function clear(): void
    {
        ...
        try {
            $finder->in($this->parameterBag->get('app.path.image_orphanage'))->date('<=' . -1 * 86400 . 'seconds')->files();
    ...
    public function initClear(SessionInterface $session): void
    {
        ...
        $path = sprintf('%s/%s', $this->parameterBag->get('app.path.image_orphanage'), $this->getPath($session));
